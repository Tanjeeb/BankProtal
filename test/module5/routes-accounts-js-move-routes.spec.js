...
describe('Move account routes', () => {
  it('`accounts.js` should contain routes @routes-accounts-js-move-routes', () => {
    ...
    assert(routeStack('/savings', 'get') === undefined, 'The savings routes has not been removed from `app.js`.');
    assert(routeStack('/checking', 'get') === undefined, 'The checking routes has not been removed from `app.js`.');
    assert(routeStack('/credit', 'get') === undefined, 'The credit routes has not been removed from `app.js`.');

    assert(R.contains('/savings', getRoutes), 'The accounts router does not contain a savings routes.');
    assert(R.contains('/checking', getRoutes), 'The accounts router does not contain a checking routes.');
    assert(R.contains('/credit', getRoutes), 'The accounts router does not contain a credit routes.');
  });
});
