...
describe('Move services routes', () => {
  it('`services.js` should contain routes @routes-services-js-move-routes', () => {
    ...
    assert(routeStack('/transfer', 'get') === undefined, 'The transfer get routes has not been removed from `app.js`.');
    assert(routeStack('/transfer', 'post') === undefined, 'The transfer post routes has not been removed from `app.js`.');
    assert(routeStack('/payment', 'get') === undefined, 'The payment get routes has not been removed from `app.js`.');
    assert(routeStack('/payment', 'post') === undefined, 'The payment post routes has not been removed from `app.js`.');

    assert(R.contains('/transfer', getRoutes), 'The services router does not contain a transfer get routes.');
    assert(R.contains('/transfer', postRoutes), 'The services router does not contain a transfer post routes.');
    assert(R.contains('/payment', getRoutes), 'The services router does not contain a payment get routes.');
    assert(R.contains('/payment', postRoutes), 'The services router does not contain a payment post routes.');
  });
});
